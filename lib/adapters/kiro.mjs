...
import { detectProject, detectTicket, matchTicket } from '../config.mjs';
...
export function scan(dir, config, processed, emit) {
  ...
  for (const file of files) {
    ...
    emit({ ts: created_at, event: 'SessionStart', session: session_id, project, ticket, cwd, source: 'kiro' });
    events++;
    ...
    if (updated_at) {
      emit({ ts: updated_at, event: 'SessionEnd', session: session_id, project, ticket, cwd, source: 'kiro' });
      events++;
    }
